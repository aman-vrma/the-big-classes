...
export function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-line pb-5">
        <div>
        ...
        <Button onClick={() => setLocation("/quiz")} className="bg-brand-600 hover:bg-brand-700 text-on-brand font-semibold shadow">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {primaryCards.map((card) => (
          <Link key={card.title} href={card.href}>
            <Card className="p-6 border-paper-line bg-paper hover:border-brand-400 hover:shadow-lg transition-all cursor-pointer flex flex-col justify-between h-full group">
              <div className="space-y-3">
                <div className={`w-12 h-12 rounded-2xl border flex items-center justify-center font-bold ${card.color}`}>
                  <card.icon className="w-6 h-6" />
                </div>
                <h2 className="font-bold text-lg text-paper-ink group-hover:text-brand-600 transition-colors">
                  {card.title}
                </h2>
              ...
              <div className="pt-6 border-t border-paper-line flex items-center justify-between text-xs font-semibold text-brand-600">
                <span>{card.cta}</span>
