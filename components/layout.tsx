...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen w-screen bg-app text-ink overflow-hidden font-sans">
      {/* Sidebar with Distinct Surface */}
      <aside className="w-64 border-r border-line/80 bg-sidebar flex flex-col justify-between shrink-0 shadow-2xl">
        <div className="p-5 space-y-6 overflow-y-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <h1 className="font-extrabold text-base text-ink tracking-wider font-serif uppercase truncate">
                THE BIG CLASSES
              </h1>
              <p className="text-[11px] font-semibold text-brand-ink truncate">
            ...
            {currentNav.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href}>
                  ...
                  >
                    <item.icon
                      // On the active pill the label sits on the accent colour, so it
                      // stays light in every theme; otherwise it follows the ink token.
                      style={{ color: isActive ? "rgb(var(--on-brand))" : "rgb(var(--ink))" }}
        ...
        {/* Sidebar Footer */}
        <div className="p-4 border-t border-line/80 bg-sidebar-deep space-y-3">
          <button
            onClick={handleExitToLogin}
            className="w-full py-2.5 px-3 rounded-xl border border-line-strong bg-surface-3/60 text-xs font-semibold text-ink hover:bg-surface-3 flex items-center justify-center gap-2 transition-all shadow-sm"
          >
            <LogOut className="w-3.5 h-3.5 text-danger-ink" />
            <span>Switch Role / Logout</span>
          </button>

          {user && (
            <Link href="/profile">
              <div className="px-2 pt-3 border-t border-line/60 flex items-center gap-2.5 cursor-pointer rounded-lg hover:bg-surface-3/40 -mx-1 px-1 py-2 transition-colors">
                <div className="w-9 h-9 rounded-full bg-brand-600/25 border border-brand-400/40 flex items-center justify-center shrink-0">
                  <span className="text-[11px] font-extrabold text-brand-ink">{initialsOf(user.name)}</span>
                </div>
                <div className="min-w-0">
                  <p className="font-bold text-ink text-xs truncate">{user.name}</p>
                  <p className="text-[11px] text-ink-soft truncate font-mono">{user.email}</p>
                  <p className="text-[10px] font-semibold text-brand-ink uppercase tracking-wider">
      ...
      <main className="flex-1 overflow-y-auto bg-app p-6 lg:p-10">
